...
| **Keystone correction** | Built, in the source, not in a release. There is no second motor on this board - the device-tree node called "keystone motor" is the focus motor (patch `0024b`) - so the correction is geometric: [`h713-warp`](docs/tools/h713-warp.md) warps the picture on the Mali-G31 with the vendor's own homography and `h713-tv` commits it on the primary plane, and [`h713-keystone`](docs/tools/h713-keystone.md) drives the eight corner values from the accelerometer. On the HY310 on 22.09.2026 a corner set by hand moved that corner on the wall, and a 25-degree nose-up tilt pulled the top edge in by itself. | Four things before it is more than beta: an hour-long soak with a live source (the one run that exists found a bug in the audio path and was not repeated); the throw ratio measured with a tape instead of taken from the optics' ini (0.8176 - if the truth is nearer 1.2, every automatic correction is too strong); the screen zoom through the warp and the projection-mode mirror RPC, both designed and not built; and the write-back self-test that would prove the identity warp pixel-exact, which needs a debug kernel and an untested vendor DMA path. The vendor's edge-blend pass is left out entirely. |
