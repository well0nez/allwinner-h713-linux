...
|---|---|---|
| **Autofocus** | Built, not proven to converge. `h713-autofocus` puts a chessboard on the panel, takes frames from the internal camera and runs the vendor's sharpness metric in C, so a run takes seconds and the motor moves ([docs/tools/h713-autofocus.md](docs/tools/h713-autofocus.md)). A run no longer ends at an edge: the search turns round there, as the vendor's does, and on 23.09.2026 runs on the bench ended at a peak (positions -2 to +31). Manual focus works as before ([docs/tools/h713-focus.md](docs/tools/h713-focus.md)). | A wall at one to three metres and a handful of runs. If the search still runs into the watcher there, the metric or the step size is wrong, not the mechanism. |
...
| **U-Boot's firmware debugging per board** | `h713_disp commdev`, `commcall`, the elog dump and the comm reader's self-test read the display firmware's structures at the HY310's addresses. On the HY300 Pro they land elsewhere (its firmware keeps its tables 0xC00 lower, A12) and the reader says "firmware not up" while the logo is on the wall (23.09.2026). The kernel side already takes the addresses from the board file (`allwinner,elog`); U-Boot has to do the same - a table per firmware identity, or the values from the board's environment - before anyone can debug a second board from its prompt. Not urgent, not to be forgotten |
