...
|---|---|---|
| **Autofocus** | Built, not proven to converge. `h713-autofocus` puts a chessboard on the panel, takes frames from the internal camera and runs the vendor's sharpness metric in C, so a run takes seconds and the motor moves ([docs/tools/h713-autofocus.md](docs/tools/h713-autofocus.md)). Every run so far ended at the motor's range watcher instead of at a peak, because the test surface stood 30 cm away. Manual focus works as before ([docs/tools/h713-focus.md](docs/tools/h713-focus.md)). | A wall at one to three metres and a handful of runs. If the search still runs into the watcher there, the metric or the step size is wrong, not the mechanism. |
| **Keystone correction** | Not built. The vendor's device-tree node called "keystone motor" turned out to be the focus motor (patch `0024b`); there is no second motor on this board. | Trapezoid correction would have to be geometric, in the display path, not mechanical. The zoom of v0.9-beta shows the firmware scaling a window of the capture ring on its own (`h713-tv ctl zoom`), but a keystone is a warp, not a window: it needs the GPU (Panfrost has not even been probed on this kernel), a warp stage between the capture ring and the plane, and a way to set the corners. Whether the MIPS firmware can be asked for it directly is unexplored. |
