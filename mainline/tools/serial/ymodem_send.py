...
def send_block(fd, blknum, payload):
    ...
    for _ in range(10):
        ...
        if r == ACK:
            return
        if r == CAN:
            raise SystemExit("receiver cancelled at blk %d" % blknum)
...
def main():
    ...
    wait_for(fd, (CRCC,), 30, "initial 'C' poll")
    send_block(fd, 0, hdr)
    wait_for(fd, (CRCC,), 30, "'C' before data phase")
    ...
    for i in range(nblocks):
        ...
        if (i + 1) % 100 == 0 or i + 1 == nblocks:
            sys.stderr.write("\r%d/%d blocks (%.1f KB/s)" %
                             (i + 1, nblocks, (i + 1) * 1.0 / max(time.time() - t0, 0.01)))
    sys.stderr.write("\n")
