...
static int sun50i_av1_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(dev->regs)) {
		dev_err(&pdev->dev, "Failed to map AV1 registers\n");
		return PTR_ERR(dev->regs);
	}

	/* Get SRAM control resource (shared CCU window, non-exclusive map) */
	res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	if (!res) {
		dev_err(&pdev->dev, "Missing SRAM control resource\n");
		return -ENODEV;
	}

	dev->sram_ctrl = devm_ioremap(&pdev->dev, res->start, resource_size(res));
	if (!dev->sram_ctrl) {
		dev_err(&pdev->dev, "Failed to map SRAM control registers\n");
		return -ENOMEM;
	}
	...
	/* Get clocks (stock-aligned: core + VE/VE3 bus + VE3 mbus) */
	dev->core_clk = devm_clk_get(&pdev->dev, "av1");
	if (IS_ERR(dev->core_clk)) {
		dev_err(&pdev->dev, "Failed to get av1/core clock\n");
		return PTR_ERR(dev->core_clk);
	}

	dev->bus_ve_clk = devm_clk_get(&pdev->dev, "bus_ve");
	if (IS_ERR(dev->bus_ve_clk)) {
		dev_err(&pdev->dev, "Failed to get bus_ve clock\n");
		return PTR_ERR(dev->bus_ve_clk);
	}

	dev->bus_ve3_clk = devm_clk_get(&pdev->dev, "bus_ve3");
	if (IS_ERR(dev->bus_ve3_clk)) {
		dev_err(&pdev->dev, "Failed to get bus_ve3 clock\n");
		return PTR_ERR(dev->bus_ve3_clk);
	}

	dev->mbus_ve3_clk = devm_clk_get(&pdev->dev, "mbus_ve3");
	if (IS_ERR(dev->mbus_ve3_clk)) {
		dev_err(&pdev->dev, "Failed to get mbus_ve3 clock\n");
		return PTR_ERR(dev->mbus_ve3_clk);
	}

	/* Get reset controls */
	dev->reset_ve = devm_reset_control_get_exclusive(&pdev->dev, "reset_ve");
	if (IS_ERR(dev->reset_ve)) {
		dev_err(&pdev->dev, "Failed to get reset_ve\n");
		return PTR_ERR(dev->reset_ve);
	}

	dev->reset_ve3 = devm_reset_control_get_exclusive(&pdev->dev, "reset_ve3");
	if (IS_ERR(dev->reset_ve3)) {
		dev_err(&pdev->dev, "Failed to get reset_ve3\n");
		return PTR_ERR(dev->reset_ve3);
	}
	...
	/* Enable power management before first MMIO access */
...
static void sun50i_av1_remove(struct platform_device *pdev)
{
