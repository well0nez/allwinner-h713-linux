...
`mainline/patches/kernel/series` carries 147 patches in fifteen sections:
...
| well0nez, follow-ups | `0005a`, `0013a`, `0014a` - `0014c`, `0024a` - `0024d`, `0078a`, `0091a`, `0091b`, `0093a`, `0096a`, `0135b`, `0136a` - `0136e` | small fixes placed directly after the original they touch, so the original stays byte-identical. `0013a` carries three decd corrections from cstenger's branch; `0005a` (USB PHY SIDDQ), `0136a` and `0136c` (HDMI hot plug) are ours, 15./16.09.; `0014b`, `0014c` and `0024d` are the msgbox's three interrupt lines and the decision that `cpu_comm` requests only its own bank's line, 16.09.; `0091a` resolves the ARISC firmware's SRAM addresses from the loaded image, 17.09.; `0096a` (the TVFE/TVCAP driver, built in), `0093a`, `0136d` (the vsync enable held while the receiver is bound) and `0091b`, `0136e` (a second hotplug pulse at the end of the bring-up) are the warm-receiver fix for issue #1, 19.09. |
