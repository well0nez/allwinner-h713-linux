...
|---|---|---|
| `u-boot` | `h713-hy310` | 33 commits on top of cstenger's `h713`: CCU tables, USB PHY, the MIPS display bring-up, the power gate, the boot paths |
...
`uboot-h713/` mirrors the U-Boot commits as patch files, generated by `release/repo-skelett.sh`, for
reading without cloning. The build uses the submodule, not the mirror.
