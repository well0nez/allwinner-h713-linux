...
| [`h713-autofocus/`](../docs/tools/h713-autofocus.md) | **The autofocus, rebuilt from the vendor's own.** One Python script, without dependencies. Chessboard on `/dev/fb0`, YUYV frames from the internal camera, the vendor's cubed-gradient metric and its five-state search; per-board step table, `run --dry-run DIR` replays recorded frames and `measure --metric-only FRAME` times the metric. | built, **not yet run on the device** (22.09.2026) |
