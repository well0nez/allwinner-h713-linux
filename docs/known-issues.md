...
| No picture after plugging a source in | The receiver sees the cable but the capture ring never starts: `INCAP meldet keine Geometrie`, the flip pointers stay put, and the automatic retry after 2 s does not help. Measured 2026-09-12 with a source behind an HDMI switch, and reproduced 2026-09-15 without a switch in automatic mode: the first plug shows the picture for about a second, then the console comes back; unplugging and plugging again makes it stay. | **`h713-tv ctl replug`** - a forced hot-plug cycle (HPD down, EDID reload, HPD up) makes the firmware take the source; it has been stable afterwards. `ctl resync` alone is not enough. |
...
| Path | State |
|---|---|
| Installer on Windows | Not tried for this release; the documentation says so explicitly. `sunxi-fel.exe` for Windows is planned after feedback from Linux users (decision 2026-09-12, `doku/116` §2). |
