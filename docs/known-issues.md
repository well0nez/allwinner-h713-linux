...
| `backlight_level` and `dynamic_backlight` do nothing | The display firmware accepts and answers both controls (41 calls, 0 unanswered) and the lamp does not change - measured at 60 and at 10 on 22.09.2026. This is the same path [dead-ends.md](dead-ends.md) already records: on the HY310 neither PWM2/PB4 nor the two backlight calls move the lamp. The controls stay for a board that wires the rail up. | none - the lamp is not adjustable on this board |
...
| A source the firmware's table does not carry takes about seven seconds to be named | The display firmware is slow to write its record for an unknown raster. Six seconds after plugging such a source in, `h713-tv` still says "change in flight"; at eight seconds it says `1600x900p60, 108.0 MHz not in the firmware's table` and shows the console, with one journal line (22.09.2026). Nothing is lost - the answer is late. |
