...
that decides whether the projector starts by itself - lives in our own fork
([well0nez/u-boot](https://github.com/well0nez/u-boot), branch `h713-hy310`, public since v0.5-beta),
not upstream mainline.
...
U-Boot is a git submodule, `mainline/external/u-boot`, pinned to our own fork of
...
his copy is inert here. A board still in test can have a branch of its own next to it - `hy300-pro-ab`
carries the panel A/B for the HY300 Pro of issue #1 and is merged into `h713-hy310` only after a green
owner report.
The submodule is a pinned commit in a fork, and `uboot-h713/` mirrors the same commits as
`git format-patch` files (`SERIES.txt` names base, head and count) - for reading, or for `git am` into
another tree. It is regenerated by hand, so `SERIES.txt` is what says which fork commit it stands on;
the fork is the authority and the build uses the submodule.
The first six commits - the H713 CCU tables, USB PHY support, the R_PIO and USB-host device-tree nodes,
the fan and USB power lines in `board_late_init`, and this board's defconfigs - are filed upstream as
...
| `hy300_pro` | the HY300 Pro of issue #1, DDR3 at the owner's own 636 MHz - TEST images only, nobody has reported a green run | 636 MHz | `sun50i-h713-hy300-pro` |
