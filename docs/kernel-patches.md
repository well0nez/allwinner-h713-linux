...
| # | Section | What it is |
|---|---|---|
| 1 | Base | the 54 patches that come through cstenger's `main`: well0nez's H713 driver series `0001` - `0022` (carried there with attribution), cstenger's arm64 side `0023` - `0048`, and our lettered follow-ups behind their originals. `0011` was withdrawn on 22.09.2026 - the file stays where it is and `series` carries the reason; `0024e` is the newest of the follow-ups and ours, not his: the GPU's operating points (288 - 696 MHz) with devfreq and a cooling map on `gpu-thermal`, and the correction of `0024`'s IOMMU comment ([video](subsystems/video.md)) |
...
| 4 | ARISC + cpu_comm | the second co-processor, the ARM↔MIPS IPC kernel API, HDMI-RX callbacks; `0091c` names the EDID version byte what it is, a per-port block mask, and lets a board state it, `0091d` records what the device then said about it (comment only), `0091e` waits for the handler's effect rather than for the classifier, because the 0x0f that byte read back was the driver's own next command overwriting the argument; `0092a` takes the display firmware's two elog addresses from the board instead of from the driver |
| 5 | Focus motor, first pass | limit-switch read path, manual move commands |
| 6 | Display path | geometry, republish-on-change, capture, picture controls, aspect; `0133a` turns the AFBD's three hardcoded handoff words into an optional device-tree property, `0133b` makes the video plane's CRTC rectangle the destination window in the descriptor the firmware scales into, `0133c` lets the plane read only the part of the ring the firmware scales into, `0133d` writes the eight window words as fractions of the firmware's fixed 30720x17280 space instead of 1/16 pixel, `0133e` puts the descriptor's source and destination window on two module parameters so both can be driven independently of the plane (empty = the record of before, word for word), and `0133f` replaces both - the parameters and `0133b`'s mapping - with the plane properties `src-window` and `dst-window` after the bench showed the eight words to be a transform into the capture ring, so the plane's CRTC rectangle is the placement on the panel and reaches no descriptor word (absent = the record of before, word for word); `0136h` - `0136k` make the display firmware's own mode table (`THDMI_ModeDetect` in `database.TSE`) the list the receiver matches against, keep a lock through our own descriptor republish, withhold a timing the firmware's picture window contradicts, and let the source cap follow that table; `0136l` - `0136q` and `0101a` translate the last German messages, give a source the table does not carry its own answer, report colour space, quantisation and the CE flag out of the firmware's record, add three more of the vendor's picture settings, narrow the own-record test, check the slot in `S_DV_TIMINGS` and correct the table's module name; `0136r` - `0136t` look the INCAP raster up in the table before the stale-record test, mark the own record in the status file and finish the driver's English; `0136y` gives the receiver `VIDIOC_EXPBUF`, so a capture slot's plane can be handed to the GPU as a `dma-buf` - a one-entry scatterlist over `dma_map_resource`, because the ring is a no-map reservation with no pages behind it, and no fence beyond the V4L2 buffer states |
...
| 15 | Boards | one DTB name per board: `0160` HY310, `0161` HY300 Pro - each an include of the bench DTS plus model and compatible; `0161a` disables the backlight device on the HY300 Pro, whose PB5 is the LED-boost and fan-rail enable as a plain GPIO; `0161b` gives that board its firmware's elog addresses. `0162` and `0163` sit at the very end of the file; `0163` makes a flip on a video plane that is already up without the ring the address slots plus DIRTY instead of the whole enable sequence, which is what the warp needs sixty times a second: it switches `twi1` on with one child, the `silan,sc7a20` accelerometer at 0x18 that the automatic keystone reads, in place of the stock tree's four candidate nodes. It is a fresh number and not a `0024f` beside `0024e` because `0082`'s board hunk carries the old `twi1` line as context and would no longer apply `0164` gives the focus motor's range gate on PH14 its pull-up, the driver's default bias (measured 23.09.2026: without a bias the pad floats and reads a stale level, the focus stood 110 msteps outside its travel unnoticed), `0165` turns homing at probe on again as stock has it, now that both edges of the gate are measured, `0166` sends the lamp's PWM parameters (channel, polarity, frequency, vsync lock, duty range) to the display firmware from `allwinner,backlight-pwm-info` in the board file before the level - the HY300 Pro's picture was dark without them (23.09.2026), the HY310 carries no property and is unchanged; `0166a` takes the PA12 pin group out again (it is the AC200's clock, and `pwm5` is no function of the pinctrl table - test8 lost its receiver over it), the six PWM words stay |
...
patch, it becomes a lettered addendum - `0005a`, `0013a`, `0014a` - `0014f`, `0024a` - `0024e`, `0078a`,
`0091a` - `0091f`, `0093a`, `0096a`, `0092a`, `0133a` - `0133f`, `0135b`, `0136a` - `0136y`, `0101a`, `0154a`, `0159a`, `0161a`, `0161b` - placed **directly behind its original**, not at the end of the
