...
| 6 | Display path | geometry, republish-on-change, capture, picture controls, aspect; `0133a` turns the AFBD's three hardcoded handoff words into an optional device-tree property; `0136h` - `0136k` make the display firmware's own mode table (`THDMI_ModeDetect` in `database.TSE`) the list the receiver matches against, keep a lock through our own descriptor republish, withhold a timing the firmware's picture window contradicts, and let the source cap follow that table |
...
`0091a`, `0091b`, `0093a`, `0096a`, `0092a`, `0133a`, `0135b`, `0136a` - `0136k`, `0159a`, `0161a`, `0161b` - placed **directly behind its original**, not at the end of the
