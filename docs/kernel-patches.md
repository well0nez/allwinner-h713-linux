...
| 4 | ARISC + cpu_comm | the second co-processor, the ARM↔MIPS IPC kernel API, HDMI-RX callbacks; `0091c` names the EDID version byte what it is, a per-port block mask, and lets a board state it, `0091d` records what the device then said about it (comment only); `0092a` takes the display firmware's two elog addresses from the board instead of from the driver |
