...
| ☐ | **Kein Pflichtabzug, wenn Mainline drauf ist** (Marco, 14.09., zum zweiten Mal). Der Installer zieht den kleinen Abzug auch vor `--restore-stock` und vor jedem Einspielen, obwohl `geraet_erkennen()` unser Layout (`hy310-*`) sicher erkennt — auf so einem Gerät gibt es nichts Stock-Eigenes mehr zu sichern, und der Abzug existiert vom ersten Einspielen. Der Abzug ist für Stock gedacht: nur verlangen, wenn die GPT Stock ist (`bootloader_a`/`super`). Einstieg `hy310-install.py` Aufrufer von `abzug_klein()` (:1107, :1134, :1153) | Befund 14.09., [`118`](118-handoff-20260913.md) §5.2 |
