...
| ☐ | **Werkzeuge sprechen Deutsch**: Schalter (`--trocken`, `--schritt`, `--abzug`, `--sicherung`) und Laufzeitausgaben (`ok`/`fehler`) sind deutsch, die Doku ist englisch. Vor P7 entscheiden: englische Aliasse dazu (billig, bricht nichts) oder umbenennen (sauber, bricht unsere Skripte). Aufgefallen bei P5 | [`117`](117-plan-p5-doku.md) |
...
| ☐ | **P7** Release: Tag `v0.1-beta`, Assets (drei Teile, Tabelle, Installer-Zip) | dito |
| ☐ | **P8** PRs an cstenger: `0022` (Env-Offset), `0143`/`0144` (pinctrl-IRQ-Bank), `0024a` (cpu_comm reg/IRQ — echter Fehler bei ihm), `0024b` (keystone→focus), `0078a` (Probe ohne 1280×720), dazu `build.sh`-Kommentarzeilen | dito |
...
| | Was | wo es steht |
|---|---|---|
| ☐ | **HDCP 1.4 über die Crypto Engine.** Weg ist geklärt und am Silizium bestätigt; offen ist der Vorabtest, ob die Non-Secure-Welt CE_S/RSSK (Key-Select 3) benutzen darf — kostet einen Stromzyklus | [`112`](112-plan-hdcp14-crypto-engine.md), [`114`](114-plan-crypto-engine.md) |
